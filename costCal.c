...
int main (int argc, const char * argv[])
{
    ...
    // Read formatted m_file
    if (fscanf(m_file, "%d", &operations) < 0)
    {
    ...
    // If error when more operations than lines - 1
    // This is due to line 1 of m_file being the operations number specifier
    if (operations > line--)
    {
    ...
    // If error when wrong producers OR consumers number
    if (consumers_number <= 0 || producers_number <= 0)
    {
        perror("ERROR: wrong producers OR consumers number");
        return -1;
    }
    ...
    // If error when queue size is smaller than 0
    if (queue_size <= 0)
    {
